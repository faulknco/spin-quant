# Experiment 19B: Activation-Calibrated Per-Row K-Means — Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Test whether weighting per-row k-means by FP32 input activation magnitudes reduces all-layer PPL, especially at lower K.

**Architecture:** Single script that (1) runs N_CALIB texts through the FP32 model with hooks to collect per-layer input activation magnitudes, (2) implements weighted k-means where each block's contribution is scaled by its activation weight, (3) quantizes all 24 MLP sublayers, (4) evaluates all-layer PPL. Compared pairwise vs unweighted at K=16, 32, 64.

**Tech Stack:** PyTorch, HuggingFace transformers, WikiText-2, `src/codebook.py` (`kmeans` for reference), `experiments/eval_perplexity.py` (`conv1d_to_linear`, `eval_perplexity`)

---

## Task 1: Create `experiments/activation_calibrated_per_row.py`

**Files:**
- Create: `experiments/activation_calibrated_per_row.py`
...
**Verify syntax:**
```bash
cd /Users/faulknco/projects/spin-quant && .venv/bin/python -c "import experiments.activation_calibrated_per_row"
```
Expected: no output, no errors.

**Commit:**
```bash
git add experiments/activation_calibrated_per_row.py
git commit -m "Add Experiment 19B: activation-calibrated per-row k-means script"
```
...
Run in background. Expected runtime: ~45–60 min (6 configs × 24 layers × per-row k-means + activation collection for calibrated configs).
...
Building: per-row K=16 uncal ...
per-row K=16 uncal       16     no   XXXXX.XXX         —

Building: per-row K=16 cal ...
per-row K=16 cal         16    yes   XXXXX.XXX     X.XXX×
...
```

**Collect results:** all 6 rows with PPL and vs-uncal ratio.

---

## Task 3: Record findings in FINDINGS_LOG.md

Open `/Users/faulknco/projects/spin-quant/FINDINGS_LOG.md` and insert the following section **before** `## Experiment 19A`. Fill in all `???`.
...
input activations x of shape [N_tokens, in_features]. Computes per-block weight
`act_weight[j] = mean(||x[:, j*d:(j+1)*d]||²)`. In weighted k-means, both distance
computation (w_j × ||block - centroid||²) and centroid update (weighted mean) use
these weights. Compares to standard unweighted per-row at K=16, 32, 64, all-layer,
bd=8.

| Config               | K  | cal | PPL      | vs uncal |
|----------------------|----|-----|----------|----------|
| per-row K=16 uncal   | 16 | no  | ???      | —        |
| per-row K=16 cal     | 16 | yes | ???      | ???×     |
| per-row K=32 uncal   | 32 | no  | ???      | —        |
| per-row K=32 cal     | 32 | yes | ???      | ???×     |
| per-row K=64 uncal   | 64 | no  | ???      | —        |
| per-row K=64 cal     | 64 | yes | ???      | ???×     |

**Key finding:** [fill in — does calibration help? at which K?]

**Implication:** [fill in — is activation weighting a useful technique?]
```

**Commit:**
```bash
git add FINDINGS_LOG.md
git commit -m "Record Experiment 19B findings: activation-calibrated per-row results"
```
...
- [ ] Task 1: `experiments/activation_calibrated_per_row.py` created and syntax-verified
- [ ] Task 1: committed
- [ ] Task 2: experiment run to completion
- [ ] Task 2: all 6 config results recorded
- [ ] Task 3: `## Experiment 19B` section inserted in FINDINGS_LOG.md before `## Experiment 19A`
